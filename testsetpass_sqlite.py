...
def tryDecryptSQLiteDB(passwordnumlist):
    for value in passwordnumlist:
        ...
        for i in range(0, TOTAL_PASS_LENGTH, 1):
            ...
            if (val >= 10):
                pwords[
                    TOTAL_PASS_LENGTH - 1 - i] = val + 87  # hex char, sqlcipher db uses lower case of abcdef, a means 10, 'a'=97, offset=97-10=87
            else:
        ...
        try:
            ...
            if str(e) == "file is not a database":
                ...
                successMark = True
                print(
                    "     The dumped database filename has already exist, so the error occur, but the password is correct.")
            else:
...
def main(argv):
    ...
    with open(Encrypted_DB_PATH, 'rb') as infile:
        ...
        encrypted_data = infile.read(
            16 * 1)  # AES Block size, 16, the minimum size to be decrypt. We only need the first 4 byte to do initial validation, so read first 16 byte.
    ...
    opencl_ctx.compile({"CONST_BYT_ACTUAL_PWLEN": str(TOTAL_PASS_LENGTH),
                        "PBKDFITER": str(PBKDF2_ITER)})  # redefine marcos in OpenCL code using new value
    ...
    for i in range(Outercycle):
        ...
        result = opencl_ctx.run(Innercycle * i, Innercycle,
                                False)  # core function, it's suggested that the Innercycle should be integer number of workgroupsize
        ...
        print("Cycle (each " + str(Innercycle) + " passphrase) " + str(i + 1) + " of " + str(
            Outercycle) + ", Time: " + str(round(time_end - time_begin, 6)) + " secs, TotalSpeed: " + str(
            round(Innercycle / 1000 / (time_end - time_begin), 2)) + " K Passphrase/s, " +
              "Estimated worst finish time: " + str(
            round((time_end - time_begin) * (Outercycle - i - 1) / 60, 1)) + " min,")

    os.system("title Brute Try Completed")
    print(" ")
    print("Brute Try completed after a total time of " + str(
        round((time.perf_counter() - program_start) / 60, 1)) + " min.")
